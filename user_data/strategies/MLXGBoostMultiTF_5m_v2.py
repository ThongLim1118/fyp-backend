...
class MLXGBoostMultiTF_5m_v2(IStrategy):
    """
    ML XGBoost multi-timeframe strategy example

    - Main timeframe: 5m
    - Informative timeframes: 15m, 1h
    - Use pretrained XGBoost model (best_xgboost_model.joblib)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        """
        df = dataframe.copy()
        if not self._logged_last_ts and len(df) > 0:
            last_ts = df.index.max()
            self._debug_log(f"[DEBUG] Last candle in strategy dataframe: {last_ts}")
            self._logged_last_ts = True
        ...
        if len(valid_idx) > 0:
            ...
            # Write predictions back to the main dataframe by index
