...
class MLPMultiTF_5m(IStrategy):
    ...
    def _predict_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        if X.size == 0:
            self._debug_log("[MLPMultiTF_5m] _predict_scores: empty X")
        ...
        if nonfinite_in > 0:
            self._debug_log(
                f"[MLPMultiTF_5m] _predict_scores: non-finite in X = {nonfinite_in}, "
        ...
        with torch.no_grad():
            ...
            self._debug_log(
                f"[MLPMultiTF_5m] raw outputs: nan={raw_nan}, "
            ...
            self._debug_log(
                f"[MLPMultiTF_5m] _predict_scores: shape={scores_np.shape}, "
    ...
    def __init__(self, config) -> None:
        ...
        model_dir = base / "models/mlp/5m/v1"
        ...
        for name, param in self.model.named_parameters():
            ...
            total_nan += nan_count
            self._debug_log(
                f"[MLPMultiTF_5m] param {name}: "
                f"nan={nan_count}, min={p.min().item():.6f}, max={p.max().item():.6f}"
            )

        self._debug_log(f"[MLPMultiTF_5m] total NaN in params: {total_nan}")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        self._debug_log(
            f"[MLPMultiTF_5m] feat_df columns sample: "
        ...
        if missing:
            self._debug_log(
                f"[MLPMultiTF_5m] Missing {len(missing)} feature columns, "
        ...
        if not available_cols:
            self._debug_log(
                "[MLPMultiTF_5m] No available ML features in feat_df, "
                "skip ML prediction."
            )
            return dataframe
        ...
        if X_scaled.size == 0:
            self._debug_log("[MLPMultiTF_5m] X is empty, no ML predictions.")
            return dataframe
        ...
        if scores.shape[0] != len(valid_idx):
            self._debug_log(
                f"[MLPMultiTF_5m] WARNING: scores len {scores.shape[0]} "
        ...
        self._debug_log(
            f"[MLPMultiTF_5m] proba: "
            f"min={proba.min():.6f}, "
            f"p50={np.percentile(proba,50):.6f}, "
            f"p90={np.percentile(proba,90):.6f}, "
            f"max={proba.max():.6f}"
        )
